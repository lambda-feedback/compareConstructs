...
def check_answer_with_output(response, correct_output, is_output_eval):
    ...
    """
    try:
        result = subprocess.run(['python', '-c', response], capture_output=True, text=True)
        if result.returncode != 0:
            return False, f"Error: {result.stderr.strip()}"
        else:
            res_output = result.stdout.strip()
    except Exception as e:
        return False, f"Exception: {e}"
    return is_different(res_output, correct_output, is_output_eval), res_output
